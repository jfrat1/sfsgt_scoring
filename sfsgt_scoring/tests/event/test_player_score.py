...
from sfsgt_scoring import event
...
def test_constructor_hole_scores_missing_indices() -> None:
    ...
    with pytest.raises(
        ...
        match="Malformed hole scores series. The index of the series should be strings 1 through 18"
    ):
        event.PlayerScore(
            course_handicap=15,
            hole_scores=hole_scores,
...
def test_get_result() -> None:
    ...
    assert all(results == pd.Series({
        "Out": 36, "In": 45, "Gross": 81, "Net": 66
    }))
