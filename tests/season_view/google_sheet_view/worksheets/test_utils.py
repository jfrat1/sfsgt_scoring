...
from season_view.google_sheet_view.worksheets.utils import process_raw_player_name, _canonicalize_player_name
...
class TestProcessRawPlayerName:
    ...
    def test_process_with_lower_case(self) -> None:
        with patch("season_view.google_sheet_view.worksheets.utils.features.FTR_CANONICALIZE_PLAYER_NAMES", True), \
             patch("season_view.google_sheet_view.worksheets.utils.features.FTR_PLAYER_NAME_CASE", NameCase.LOWER):
            result = process_raw_player_name("Doe, John")
            assert result == "john doe"
